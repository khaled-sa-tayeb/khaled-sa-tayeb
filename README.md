<div align="center">

<img src="https://github.com/SP-XD/SP-XD/blob/main/images/hellocoders_rounded.gif?raw=true" width="55%">

# Hi, I'm Khaled 👋

### Computer Science Graduate • Data Analyst • AI Enthusiast

<p>
<a href="https://github.com/kahmed0018-hub">
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>
<a href="https://linkedin.com/in/khaled-tayeb0066/">
<img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>
</p>

</div>

---
<h2>🚀 About Me</h2>

<p>
Computer Science graduate with hands-on experience in <strong>data analytics, machine learning, NLP, and AI-powered systems development</strong>. I enjoy working with large and complex datasets, building data-driven solutions, and turning raw information into meaningful insights and practical applications.
</p>

<ul>
  <li>🤖 Built <strong>AI-Powered External Auditor</strong> — an AI-based auditing system for analyzing financial data, detecting discrepancies, and validating invoice information using OCR</li>

  <li>📊 Analyzed and cleaned <strong>1M+ records</strong> during my internship at the <strong>Data Management Office – King Abdulaziz University</strong></li>

  <li>📈 Built interactive <strong>Power BI dashboards</strong> to support data-driven decision-making</li>

  <li>🧠 Developed a <strong>machine learning classification model</strong> and applied <strong>NLP techniques</strong> to text data</li>

  <li>🐍 Experienced with <strong>Python, Pandas, NumPy, Machine Learning, NLP, and Data Visualization</strong></li>

  <li>🗄️ Designed and implemented a relational <strong>University Library Database</strong> using SQL Server, including schema design, normalization, and query optimization</li>

  <li>🎮 Developed <strong>Leap Hero</strong>, a 2D platformer inspired by <em>Jump King</em> and <em>Celeste</em> using Unity</li>

  <li>🎓 <strong>B.Sc. in Computer Science — King Abdulaziz University</strong></li>

  <li>🏆 <strong>GPA: 4.75/5.00 — First Class Honors</strong></li>

  <li>📚 Completed courses in <strong>Exploratory Data Analysis for Machine Learning</strong> and <strong>Linear Algebra for Machine Learning &amp; Data Science</strong></li>

  <li>🚀 Passionate about <strong>Data Analysis, Artificial Intelligence, Machine Learning, and building practical software solutions</strong></li>
</ul>

## 🧰 Tech Stack

### 💻 Programming

<p>
<img src="https://skillicons.dev/icons?i=python,java,js,sql" />
</p>

### 🤖 Data & AI

![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge\&logo=pandas\&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=for-the-badge\&logo=numpy\&logoColor=white)
![Machine Learning](https://img.shields.io/badge/Machine%20Learning-102230?style=for-the-badge\&logo=scikitlearn\&logoColor=white)
![NLP](https://img.shields.io/badge/NLP-8A2BE2?style=for-the-badge\&logo=google\&logoColor=white)

### 📊 Analytics & Tools

![Power BI](https://img.shields.io/badge/Power%20BI-F2C811?style=for-the-badge\&logo=powerbi\&logoColor=black)
![Excel](https://img.shields.io/badge/Excel-217346?style=for-the-badge\&logo=microsoftexcel\&logoColor=white)
![SQL Server](https://img.shields.io/badge/SQL%20Server-CC2927?style=for-the-badge\&logo=microsoftsqlserver\&logoColor=white)
![MATLAB](https://img.shields.io/badge/MATLAB-0076A8?style=for-the-badge\&logo=mathworks\&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge\&logo=git\&logoColor=white)
![Unity](https://img.shields.io/badge/Unity-000000?style=for-the-badge\&logo=unity\&logoColor=white)

---

## 🚀 Featured Project

### 🤖 AI-Powered External Auditor

**AI-powered financial auditing system** developed as my senior project.

The system analyzes financial data and helps identify discrepancies through automated auditing processes.

**Key Highlights**

* 🐍 Built data preprocessing pipelines using **Python, Pandas & NumPy**
* 🧾 Integrated **OCR** for invoice data extraction
* 🔍 Validated extracted invoice data against structured records
* 📊 Developed a web-based interface for visualization and interaction
* 🤖 Applied AI-based approaches to automate parts of the auditing process

---

## 💼 Experience

### 📊 Data Management Office — King Abdulaziz University

**Internship · May 2025 – Aug 2025**

* Analyzed and cleaned **1M+ records**
* Built **Power BI dashboards** to support decision-making
* Developed a machine learning classification model
* Applied **NLP techniques** to text data

### 💻 Pure Platform Company

**Internship · Dec 2023 – Jan 2024**

* Supported software development activities
* Worked with databases and backend operations
* Collaborated within a development team

---

## 📚 Education

**Bachelor of Science in Computer Science**
King Abdulaziz University

**GPA:** 4.75 / 5.00
**First Class Honors**

---

## 📌 Other Projects

### 🗄️ Central Library Database

Relational database system designed for a university library using **SQL Server**, focusing on schema design, normalization, and query optimization.

### 🎮 Leap Hero

A 2D platformer inspired by **Jump King** and **Celeste**, developed with **Unity**, focusing on character physics, level design, and user input systems.

---

## 📈 GitHub Stats

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=kahmed0018-hub&show_icons=true&theme=tokyonight&hide_border=true&rank_icon=github" width="48%"/>

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=kahmed0018-hub&layout=compact&theme=tokyonight&hide_border=true" width="40%"/>

</div>

---

<div align="center">

### 💡 Build • Learn • Analyze • Solve

⭐ Thanks for visiting my profile!

</div>
